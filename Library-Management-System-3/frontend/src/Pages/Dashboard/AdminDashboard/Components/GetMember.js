...
function GetMember() {
    ...
    useEffect(() => {
        ...
        const getMembers = async () => {
            try {
                const { data } = await axios.get(`${API_URL}api/users/allmembers`, {
                    signal: controller.signal, // Pass signal to axios request
                });
                setAllMembersOptions(data.map((member) => ({
                    value: member?._id,
                    text: `${member?.userFullName} [${
                        member?.userType === "Student" ? member?.admissionId : member?.employeeId
                    }]`,
                })));
            } catch (error) {
        ...
        return () => controller.abort(); // Cleanup: abort request when component unmounts
    }, [API_URL]);
    ...
    useEffect(() => {
        ...
        const getMemberDetails = async () => {
            if (memberId !== null) {
                try {
                    const response = await axios.get(API_URL + "api/users/getuser/" + memberId, { signal });
